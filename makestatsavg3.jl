...
function main()
    ...
    @inbounds for l in eachindex(statsdata)
        meanstats[l] = mean(view(statsdata[l],202:402))
    end

    open("StatsAvg3.txt",write=true) do io
    ...
    @inbounds for l in eachindex(scalesdata)
        meanscales[l] = mean(view(scalesdata[l],202:402))
    end

    open("ScalesAvg3.txt",write=true) do io
    ...
    @inbounds for l in eachindex(numbersdata)
        meannumbers[l] = mean(view(numbersdata[l],202:402))
    end

    open("NumbersAvg3.txt",write=true) do io
